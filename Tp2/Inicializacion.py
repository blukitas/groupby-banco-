...
class Inicializacion:
    ...
    param_xgboost_grid = {'n_estimators':np.arange(1200,1400,100),
    ...
    def operaciones(self, df):
        ...
        # TODO: !! Log en precio y exponencial en la salida.
        return df
    ...
    def encoding(self, df):
        ...
        if not self.paramsGenerales['esTest']:
            self.tiposPrincipales = df.tipodepropiedad.value_counts().sort_values(kind="quicksort",
    ...
    def features_engineering(self, df):
        ...
        zonas_exclusivas = ['Lomas de Chapultepec', 'Polanco', 'Bosques de las Lomas', 'Colinas del Bosque',
    ...
    def recast(self, df):
        ...
        cols = [x for x in df.columns if
                x != 'precio' and x != 'mes_sin' and x != 'mes_cos' and x != 'construccion_density' and x != 'prom_amb' and
                df[x].dtype.type == np.float64]
        print(cols)
        for x in cols:
            if df[x].isnull().sum() > 0:
                pass
    ...
    def scaler(self, df):
        ...
        # TODO: Distintos scaler, pruebas. Entrenar con train, guardar modelo, si se pudiera y usarlo contra test.
        # if not self.paramsGenerales['esTest']:
        #     self.stdScaler = StandardScaler()
        #     self.stdScaler.fit(df)
...
# TODO: Ensambles: bagging,boosting y stacking son las 3 formas de ensamblar
